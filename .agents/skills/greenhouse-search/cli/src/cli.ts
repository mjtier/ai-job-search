...
const HELP = `greenhouse-cli — search Greenhouse public job boards (greenhouse.: one board per company slug)
...
SEARCH FLAGS
  ...
  --company, -c <slugs>  Comma-separated Greenhouse board slugs. Repeatable.
...
DETAIL
  ...
  <board url>            A Greenhouse posting URL.

EXAMPLES
  bun run src/cli.ts search -q "program manager" --limit 10 --format table
  bun run src/cli.ts search -q "solutions architect" -c stripe --jobage 30 --format table
  bun run src/cli.ts search -l "Florida" --format table
  bun run src/cli.ts detail stripe:12345 --format plain
