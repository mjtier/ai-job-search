...
// configured slug list and filters client-side. Verified live against boards-api.greenhouse.io.

export const PROVIDER = "greenhouse"

export function writeError(error: string, code: string): void {
  process.stderr.write(JSON.stringify({ error, code }) + "\n")
}

const UA = "Mozilla/5.0 (compatible; greenhouse-search-cli/1.0)"
...
export function normalize(company: string, raw: any): JobCard {
  const loc = raw?.location?.name ?? null
  const stamp: string | null = raw?.first_published ?? raw?.updated_at ?? null
  return {
    id: `${company}:${raw?.id}`,
...
export function describe(raw: any): Omit<JobDetail, keyof JobCard> {
  // Greenhouse double-encodes the description: HTML entities wrapping HTML tags.
  const decoded = decodeEntities(String(raw?.content ?? ""))
  return {
    description: stripHtml(decoded) || null,
    employmentType: null,
    department: Array.isArray(raw?.departments) && raw.departments.length
      ? String(raw.departments[0]?.name ?? "") || null
      : null,
  }
}
