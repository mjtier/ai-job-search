...
version: 1.0.0
description: >
  Use this skill to search live job listings hosted on Ashby job boards — the
  ...
  Ashby jobs, jobs at <company>, Ashby board, open roles at <company>,
...
Search live job postings from **Ashby** public job boards. Ashby hosts one
...
> Because Ashby is an ATS rather than a job portal, there is no global search
...
Default slugs live in `.agents/skills/ashby-search/companies.txt`, one per
...
bun run .agents/skills/ashby-search/cli/src/cli.ts detail <company>:<id> [--format json|plain]
```
...
bun run .agents/skills/ashby-search/cli/src/cli.ts search --jobage 14 --format table

# Program management roles, any target company
bun run .agents/skills/ashby-search/cli/src/cli.ts search -q "program manager" --format table

# One employer, filtered to Florida
bun run .agents/skills/ashby-search/cli/src/cli.ts search -c ramp -l "FL" --format table

# Remote-friendly senior roles posted this month
bun run .agents/skills/ashby-search/cli/src/cli.ts search -q "senior" -l "remote" --jobage 30 --format table

# Read one posting in full
bun run .agents/skills/ashby-search/cli/src/cli.ts detail ramp:12345 --format plain
```
