...
const HELP = `lever-cli — search Lever public job boards (lever.: one board per company slug)
...
SEARCH FLAGS
  ...
  --company, -c <slugs>  Comma-separated Lever board slugs. Repeatable.
...
DETAIL
  ...
  <board url>            A Lever posting URL.

EXAMPLES
  bun run src/cli.ts search -q "program manager" --limit 10 --format table
  bun run src/cli.ts search -q "solutions architect" -c palantir --jobage 30 --format table
  bun run src/cli.ts search -l "Florida" --format table
  bun run src/cli.ts detail palantir:12345 --format plain
