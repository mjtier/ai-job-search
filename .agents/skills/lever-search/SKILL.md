...
version: 1.0.0
description: >
  Use this skill to search live job listings hosted on Lever job boards — the
  ...
  Lever jobs, jobs at <company>, Lever board, open roles at <company>,
...
Search live job postings from **Lever** public job boards. Lever hosts one
...
> Because Lever is an ATS rather than a job portal, there is no global search
...
Default slugs live in `.agents/skills/lever-search/companies.txt`, one per
...
bun run .agents/skills/lever-search/cli/src/cli.ts detail <company>:<id> [--format json|plain]
```
...
bun run .agents/skills/lever-search/cli/src/cli.ts search -q "program manager" --format table

# One employer, filtered to Florida
bun run .agents/skills/lever-search/cli/src/cli.ts search -c palantir -l "FL" --format table

# Remote-friendly senior roles posted this month
bun run .agents/skills/lever-search/cli/src/cli.ts search -q "senior" -l "remote" --jobage 30 --format table

# Read one posting in full
bun run .agents/skills/lever-search/cli/src/cli.ts detail palantir:12345 --format plain
```
